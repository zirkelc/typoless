...
/**
 ...
 On the main actor. A field in another app is reached by messaging that app, so
 the thread these calls run on did not seem to matter, and they ran on whatever
 thread the async call landed on. A field in this app's own windows is
 different: the accessibility call goes straight into AppKit on the calling
 thread, and AppKit traps when a text view is edited off the main thread. That
 made the field in the setup window crash the app the first time it was ever
 corrected. The waits between writes suspend rather than block, but each
 accessibility call is synchronous, so a target app that stops answering holds
 the main thread while the call waits. The resolver caps that wait for every
 call this app makes.
 */
@MainActor
enum TextWriter {
    ...
    private static func applyIndividually(
        ...
        /**
         ...
         `TYPOLESS_FORCE_WHOLE_VALUE=1` set returns every word to the default
         colour and size; without it the styling survives untouched.
         */
        if ProcessInfo.processInfo.environment["TYPOLESS_FORCE_WHOLE_VALUE"] != nil {
...
enum TextWriteError: Error {
    ...
    var userMessage: String {
        switch self {
        case .ineffective:
            return "Typoless could not write the correction back into that field."
        case .diverged:
            return "Typoless stopped because that field changed while it was being corrected."
        case .wouldFlattenField:
            return """
