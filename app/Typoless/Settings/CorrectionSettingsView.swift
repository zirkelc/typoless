    ...
    var body: some View {
        ...
        .onAppear {
            /**
             ...
             include text that has already passed its retention. Runs on every
             visit: the page is kept, but it appears again each time its tab is
             chosen.
             */
            history.prune()
            ...
            /**
             ...
             page is for, but the page is kept between visits and appears again
             each time, so seeding on every appearance re-expanded the first
             language after the user had deliberately collapsed them all.
             */
        ...
        VStack(spacing: 0) {
            ...
            if isExpanded {
                ...
                ForEach(Array(language.rules.enumerated()), id: \.element.rule) { index, example in
                    if index > 0 {
                        Divider().padding(.horizontal, SettingsRow.horizontalPadding)
                    }
