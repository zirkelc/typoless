...
final class AppModel {
    ...
    /**
     Nil where the app was built without a feed or a signing key, which is
     every Debug build: the feed is set only in Release. Starting Sparkle then
     would check against a feed that may not exist yet, and would offer to
     replace a development build with the last release.
     */
    ...
    /**
     Starts a correction, unless the app is paused or not set up.

     Checked here rather than at each caller, because the callers are several
     (the menu item, the hot key, the double-tap, the setup window's button)
     and a pause that only some of them respect is not a pause. The setup
