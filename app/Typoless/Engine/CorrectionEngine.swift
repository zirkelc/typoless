...
final class CorrectionEngine {
    ...
    func run() async {
        ...
        do {
            ...
            report("Typoless could not correct that text.", log: "Corrector failed: \(error)")
            return
        }
    ...
    func revertLast() async {
        ...
        guard let current = fix.element.string(kAXValueAttribute), current == fix.after else {
            report(
                "That text has changed since Typoless corrected it.",
        ...
        do {
            ...
            report("Typoless could not undo that correction.", log: "Revert failed: \(error)")
            return
        }
